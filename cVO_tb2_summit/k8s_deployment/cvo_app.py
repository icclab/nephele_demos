...
async def read_property_from_tb2():
    ...
    possibleSummitLaunchfiles_tb2 = await consumed_vos["vo1"].properties['possibleLaunchfiles_tb2'].read()
    
    # Initialize the property values
    await exposed_thing.properties['allAvailableResources_tb2'].write(allAvailableResources_tb2)
    await exposed_thing.properties['possibleLaunchfiles_tb2'].write(possibleSummitLaunchfiles_tb2)
...
async def read_property_from_summit():
    ...
    possibleSummitLaunchfiles_summit = await consumed_vos["vo2"].properties['possibleLaunchfiles_summit'].read()
    
    # Initialize the property values
    await exposed_thing.properties['allAvailableResources_summit'].write(allAvailableResources_summit)
    await exposed_thing.properties['possibleLaunchfiles_summit'].write(possibleSummitLaunchfiles_summit)
